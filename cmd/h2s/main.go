...
func NewProxyServer(conf []byte) (*Server, error) {
	cfg := &TConf{}
...
func main() {
	var addr, confPath string
	var config []byte
	var err error
	flag.StringVar(&addr, "addr", "0.0.0.0:3128", "Which Addr the proxy listens")
	flag.StringVar(&confPath, "c", "", "Config json path")
	flag.Parse()
